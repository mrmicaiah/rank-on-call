...
**Status:** §1 decisions LOCKED and being built against (see `docs/FULFILLMENT_WORKER_SPEC.md`); §§2–9 are reference architecture, still open to refinement.
...
2. **CONSEQUENCE — the live copy must change. ⚠️ AMENDED 2026-08-01: this gate MOVED.** The site promises per-report human review, in two places, verbatim: `src/index.njk:72` — *"The method behind these reports was built by hand … The research runs automatically. What it looks for doesn't. And nothing goes out unread — a person reads every report before it reaches your inbox."* — and `src/thank-you.njk:63` — *"Every report gets read by a person before it's sent — that's what the time is for."* `src/index.njk` also carries a claim-ceiling comment recording per-report human review as a committed feature. Full auto-send makes that copy FALSE, and it is a deliberate product reversal — not an implementation detail.

   **What changed (recording Irene's decision of 2026-08-01, not reopening it):** this item previously read *"live copy must change before this ships … flag to Irene at ship time."* The site has no traffic, is not indexed, and is not advertised, so the copy is now built **in its final form immediately** rather than deferred. The gate did not disappear — it moved from *"copy must not ship"* to *"traffic must not be pointed at the site."*

   **The STANDING RULE in the front matter of `docs/FUNNEL_REORDER_SPEC.md` is authoritative** and lists the conditions that must all hold before traffic. Deliberately not restated here — one source of truth. ⚠️ **Shipped copy is not permission to launch.**
