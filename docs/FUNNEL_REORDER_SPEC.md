...
7. ~~**§7.1 DataForSEO billing coupling gets sharper.** A balance drained by an unrelated project at 2am used to have three days of slack to be noticed and topped up. It now has hours.~~ **✅ RESOLVED 2026-08-01 — moot.** Irene created a **completely separate DataForSEO account** for ROC, so no unrelated project can reach the balance at all. `FULFILLMENT_WORKER_SPEC.md` §7.1. **One item survives the resolution:** the new account holds only its $1 signup credit and **must be funded before the first live report** — a hard prerequisite before traffic, alongside the standing rule's four conditions.
